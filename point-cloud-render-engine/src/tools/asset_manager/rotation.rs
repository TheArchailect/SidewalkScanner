...
pub fn rotate_active_bounds_on_scroll(
    ...
    mut q: Query<(&mut Transform, &mut PlacedAssetInstance), (With<ActiveRotating>, With<Selected>)>,
    ...
    for (mut transform, mut placed_instance) in &mut q {
        ...
        if let Some(global_instance) = placed_assets.instances.iter_mut().find(|inst| {
            ...
            inst.transform.translation.distance(placed_instance.transform.translation) < 0.1
        }) {
        ...
        if let Some(manifest) = assets.manifest.as_ref().and_then(|h| manifests.get(h)) {
            if let Some(asset_meta) = manifest.asset_atlas.as_ref().and_then(|aa| aa.assets.first()) {
                ...
                let new_data: Vec<crate::engine::render::instanced_render_plugin::InstanceData> =
                    placed_assets.instances.iter().map(|placed| crate::engine::render::instanced_render_plugin::InstanceData {
